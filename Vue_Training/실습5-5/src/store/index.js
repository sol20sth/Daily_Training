...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    LOAD_ORDER(state) {
      const sessionStorageOrder = sessionStorage.getItem("orderList");
      const parsedOrder = JSON.parse(sessionStorageOrder) || [];
  ...
  actions: {
    ...
    addOrder(context) {
      ...
      context.dispatch("saveOrderSessionStorage");
    },
    ...
    saveOrderSessionStorage(context) {
      const jsonOrder = JSON.stringify(context.state.orderList);
      sessionStorage.setItem("orderList", jsonOrder);
    },
    loadOrder(context) {
      context.commit("LOAD_ORDER", context);
    },
	resetAll(context) {
		context.commit("RESET_ALL", context);
	}
  },
  modules: {},
});
